...
st.set_page_config(page_title='Chat with Doc', page_icon='🤖', layout='wide')


# Function to extract model names from the provided model information
@st.cache_data
def extract_model_names(_models_list: List[Dict[str, Any]]) -> Tuple[str, ...]:
    """
    Extracts model names from a list of model dictionaries.

    Args:
        models_list (List[Dict[str, Any]]): List containing model details as dictionaries.

    Returns:
        Tuple[str, ...]: Tuple of model names.
    """
    return tuple(model.get("name", model.get("model")) for model in _models_list if "model" in model)

# ✅ Fetch models from Ollama safely
try:
    models_info = ollama.list()  # returns something like {"models": [...]}
    models_list = models_info.get("models", [])  # safely get list or empty list
    available_models = extract_model_names(models_list)
...
def chat_generate(model, prompt, options, context=None):
    ...
    if context:
        ...
        response = ollama.generate(model=model, prompt=f'Using this data: {context}. Respond to this prompt: {prompt}',
                                   stream=False, keep_alive=0, options={**options, "use_mmap": True})
        ...
        response = ollama.generate(
                    ...
                    stream=False,
                    keep_alive=0,
                    options={**options, "use_mmap": True}  # Ensures use of local model
                )
    ...
    if (is_new_document or is_new_model or is_new_chunk_params or is_new_llm_params or
        ...
        try:
            ...
            for i, chunk in enumerate(docs):
                ...
                response = ollama.embeddings(model=embedding_model, prompt=chunk, options={"use_mmap": True})
...
def main() -> None:
    ...
    st.title('🤖 RAG based Document Chatbot', anchor=False)
    st.divider()
    ...
    with st.sidebar:
        ...
        available_models = extract_model_names(models_info.get("models", []))
